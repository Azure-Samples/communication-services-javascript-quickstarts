...
- Azure Communication Services real-time voice and video client calling SDK can be used for a wide range of Business To Consumer calling scenarios. Your application benefits from the wide ecosystem available on Azure cloud services.
- This sample is an integration of Azure Communication Services Client Calling SDK with Azure Open AI to show how businesses could benefit from artificial intelligence in a client-calling flow. 
- This sample demonstrates how a customer support agent receive live talking point recommendations from Azure Open AI on what to say next to best support the customer. 
- This sample also shows end of call customer satifaction (sentiment) and end call summary generated by Azure Open AI that results in better insight for businesses to understand customer satisfaction.


# Example scenario:
Imagine a Contoso Live support agent resolving customer issues. During the call, the support agent needs to have a stronger understanding of the customer issues, and come up with troubleshooting suggestions. The support agent must also gather customer details and fill in a form to create a support ticket.

 Businesses wish to learn about quality of their customer support calls. This sample project shows how real-time intelligence can support a live agent and the business with insights:

1.	Contoso customer calls the Contoso business support agent who is using Azure Communication Services calling SDK using a Team’s identity. 
2.	The Contoso support agent’s client application receives the [Closed captions](https://learn.microsoft.com/en-us/azure/communication-services/concepts/interop/enable-closed-captions) of the current conversation between the customer and the agent from Azure Communication Services Client Calling SDK. 
3.	The closed captions are sent from the client application to a back-end service built with Azure functions, that is set up to act as a gateway to Azure Open AI. Two prompts are set, one to get Contoso agent suggestions along with customer data gathered from the conversation and the other prompt is to gather call sentiment and insights.
4.	The suggestions received from the Azure Open AI are shown to the customer support agent. These suggestions give hints to support the agent in solving the customer's issue. The customer information gathered by AI and is used to fill in support ticket form. 
5.	Business insights such as call sentiment (positive/neutral/negative) and call insight summary are sent from the back-end service to Azure monitor, and logs analytics for business visualization.
