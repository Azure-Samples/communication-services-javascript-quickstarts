...
const formContextItems = (context: ContextItem[]): MessageCreateParams[] => {
  ...
  context.forEach((item) => {
    ...
    messages.push({
      role: item.senderName === "assistant" ? "assistant" : "user",
      content,
    });
  });
  return messages;
...
export const askAI = async (
  ...
  try {
    ...
    const assistant = await azureOpenAIClient.beta.assistants.create({
      name: "Assistant",
      instructions:
        "You are a personal assistant to help me facilitate the chat.",
    ...
    const run = await azureOpenAIClient.beta.threads.runs.createAndPoll(
      thread.id,
      {
    ...
    if (run.status === "completed") {
      const messages = await azureOpenAIClient.beta.threads.messages.list(
        run.thread_id
      );
